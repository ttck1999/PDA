...
function ajaxData(scanCode,secondCode){
	...
	$.ajax({
        url:pathUrl()+'/spd-sys/admin/spd/spdWhSerial/scanMedInfo',
        beforeSend: function(request) {
            request.setRequestHeader("PADSESSION", sessionStorage.getItem("PADSESSION"));
        },
        type:"post",
        data:{'scanCode':scanCodeTmp,"secondCode":$.trim(secondCode),"deptCode":JSON.parse(sessionStorage.getItem('parentObj')).deptCode,"whCode":JSON.parse(sessionStorage.getItem('parentObj')).whCode},
        dataType:'json',
...
function numData(scanCode,secondCode,n){
	$.ajax({
		url:pathUrl()+'/spd-sys/admin/spd/spdWhSerial/scanMedStoreNum',
		beforeSend: function(request) {
		    request.setRequestHeader("PADSESSION", sessionStorage.getItem("PADSESSION"));
		},
		type:"get",
		data:{"scanCode": scanCode,"secondCode":secondCode,"deptCode":JSON.parse(sessionStorage.getItem('parentObj')).deptCode,"num":n,"whCode":JSON.parse(sessionStorage.getItem('parentObj')).whCode},
		dataType:"json",
		success:function(data) {
			objData = data;
